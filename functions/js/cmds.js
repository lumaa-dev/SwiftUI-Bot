	 ...
	 */
	 async initiate(client, message, hasCustomData = true) {
		...
		if (
			...
			await client.guilds.cache.get(message.guild.id)?.commands.set(/*convert(a)*/ a);
...
/**
 * Convert object[] to SlashCommandBuilder[]
 * @deprecated Useless
 * @param {{name: String, description: String, options: [{ name: String, description: String, type: String }]}[]} cmds 
 */
			...
			switch (type) {
				...
				case "INTEGER":
					builder.addIntegerOption({ 
						...
						description: option.description,
						required: option.required ?? true,
						min_value: option.min_value ?? 1,
						max_value: option.max_value ?? 9999,
						choices: option.choices ?? []
					})
					break;
